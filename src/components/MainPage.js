...
function CountUp({ end, duration = 2, ...props }) {
  ...
  return <span {...props}>{count.toLocaleString()}{!finished && '+'}</span>;
}

const campusImg = 'https://dimajadid.com/wp-content/uploads/2023/07/image-70.png';
...
function MainPage() {
  ...
  return (
    <div className="homepage">
      {/* Accueil / Bannière */}
      <section className="banner" id="accueil" style={{backgroundImage: 'url(/m.png)', backgroundSize: 'cover', backgroundPosition: 'center', position: 'relative'}}>
        <div style={{position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(230,80,30,0.35)', zIndex: 1}}></div>
        <div className="banner-content" style={{position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'center', minHeight: 380, width: '100%', paddingLeft: 80}}>
          <h1 style={{color: '#fff', fontWeight: 'bold', fontSize: '3rem', marginBottom: 18, textAlign: 'left'}}>Université Mohammed VI Polytechnique</h1>
          <h2 style={{color: '#fff', fontWeight: 400, fontSize: '1.5rem', marginBottom: 32, textAlign: 'left'}}>Innovation, Recherche et Excellence</h2>
          {/* Boutons supprimés */}
        </div>
      </section>
